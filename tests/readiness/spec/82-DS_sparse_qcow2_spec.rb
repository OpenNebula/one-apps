...
RSpec.describe "DS Operations TM_MAD = qcow2 with SPARSE = NO" do
    before(:all) do
        ...
        wait_loop(:success => true) {
            cmd = cli_action("oneimage show #{@info[:img_qcow2_id]} 2>/dev/null", nil)
            cmd.fail?
        }
    end
    ...
    after(:all) do
        ...
        # Delete image
        cli_action("oneimage delete #{@info[:img_qcow2_id]}")
        wait_loop(:success => true) {
            cmd = cli_action("oneimage show #{@info[:img_qcow2_id]} 2>/dev/null", nil)
            cmd.fail?
        }
    ...
    it "creates 256MB qcow2 DATABLOCK image" do
        cmd = "oneimage create " <<
              "--name qcow2_image_#{@info[:vm_id]} " <<
              "--type DATABLOCK " <<
              "--format qcow2 " <<
              "--size #{@info[:img_size]} " <<
              "-d #{@info[:ds_id]}"

        @info[:img_qcow2_id] = cli_create(cmd)

        wait_loop(:success => "READY", :break => "ERROR") {
            xml = cli_action_xml("oneimage show -x #{@info[:img_qcow2_id]}")
    ...
    it "attachs qcow2 DATABLOCK image to VM" do
        cli_action("onevm disk-attach #{@info[:vm_id]} --image #{@info[:img_qcow2_id]} --prefix #{@info[:prefix]}")
        @info[:vm].running?
    end

    it "checks size of qcow2 disk on created vm is same as assigned" do
        real_size = 0
