...
require_relative 'equinix/cleanup'
...
RSpec.shared_examples_for 'provision' do |hypervisor,
                                          instance_type,
                                          provider_path,
                                          inputs|
    before(:all) do
        ...
        when 'equinix'
            $provider['connection']['token']   = credentials['packet_token']
            $provider['connection']['project'] = credentials['packet_project']
        when 'aws'
            $provider['connection']['access_key'] = credentials['aws_access']
            $provider['connection']['secret_key'] = credentials['aws_secret']
        when 'digitalocean'
            $provider['connection']['token']  = credentials['do_token']
            $provider['connection']['region'] = credentials['do_region']
        when 'google'
            $provider['connection']['credentials'] = credentials['credentials']
            $provider['connection']['project']     = credentials['project']
        when 'vultr_virtual', 'vultr_metal'
            $provider['connection']['key'] = credentials['key']
        end
    ...
    it 'should delete resources on remote provider' do
        ...
        when 'digitalocean'
            p = DO.new(connection['token'])
        when 'google'
            p = GoogleP.new(connection['credentials'], connection['project'])
        when 'vultr_virtual', 'vultr_metal'
            p = VultrP.new(connection['key'])
        end
    ...
    it 'should configure a RUNNING provision' do
        cmd = cli_action_timeout(
            "oneprovision configure #{@info[:p_id]} -D --force --batch --fail-modes cleanup", true, 1800
        )
    ...
    it 'should add one more host to the provision', :required do
        cmd = cli_action_timeout(
            "oneprovision host add #{@info[:p_id]} -D --batch --fail-modes cleanup", true, 1800
        )
    ...
    it 'should create VM template', :required do
        ...
        when 'firecracker'
            cli_create(
                ...
                "-d #{$provider['provider']}-edge-cluster-image " \
    ...
    it 'should update net to use new sg' do
        ...
        cli_action(
            "onevnet update #{$provider['provider']}-edge-cluster-public " \
            "-a #{tpl.path}"
        )
    end
    ...
    it 'should attach NIC to all VMs', :required do
        VMS.each do |vm|
            ...
            @info[vm].nic_attach("#{$provider['provider']}-edge-cluster-public")
    ...
    it 'should create private vnet' do
        ...
        @info[:vnet_private] = cli_create(
            "onevntemplate instantiate #{$provider['provider']}-edge-cluster-private"
        )
    ...
    it 'should attach NIC alias' do
        ...
        @info[:vm1].nic_attach(
            "#{$provider['provider']}-edge-cluster-public --alias NIC0"
        )
