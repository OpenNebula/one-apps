require 'json'
require 'init'
require_relative '/var/lib/one/readiness/lib/image'
require_relative '/var/lib/one/readiness/lib/backup_restic'
require_relative '/var/lib/one/readiness/lib/backup_rsync'
require_relative '/var/lib/one/readiness/lib/VMTemplate'
...
RSpec.describe 'KVM FireEdge', :type => 'skip' do
  ...
  Dir.chdir(dir_tests) {
    ...
    # Create datastores before Cypress tests (before(:all) not works with Cypress tests) 
    ResticDS.create('DS_RESTIC', Host.private_ip)
    RSyncDS.create('DS_RSYNC', Host.private_ip)
