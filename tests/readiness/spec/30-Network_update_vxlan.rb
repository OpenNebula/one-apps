...
RSpec.describe 'Virtual Network Update vxlan' do
    ...
    it 'Update MTU' do
        vlan_link = 'tapup0.111'

        info = @info[:vn].link_info(vlan_link)
    ...
    it 'Update VLAN_ID' do
        info = @info[:vn].link_info('tapup0.111')

        expect(info['master']).to eq 'onebr.up'
        ...
        info = @info[:vn].link_info('tapup0.222')
    ...
    it 'Update PHYDEV' do
        ...
        expect(ports.include?('tapup1.222')).to be(false)
        ...
        expect(ports.include?('tapup1.222')).to be(true)
    end
