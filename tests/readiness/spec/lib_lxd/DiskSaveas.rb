shared_examples_for "DiskSaveas" do |livesnap|
    ...
    it "create non-pers datablock" do
        cmd = "oneimage clone #{@info[:rootfs_disk_id]} non_pers_#{@info[:vm_id]}"
        ...
        cmd = "oneimage nonpersistent #{img_id}"

        @info[:img_id] = img_id
    end

    it "create datablock" do
        cmd = "oneimage clone #{@info[:rootfs_disk_id]} pers_#{@info[:vm_id]}"
        ...
        cmd = "oneimage persistent #{img_id}"
...
######Snapshot was not created because it is not supported, so disk-saveas -s (snapshot) 0 does not exist.
#    it "save snap non-pers" do
#        @info[:img_id_save_s1] = cli_create("onevm disk-saveas #{@info[:vm_id]} #{@info[:disk_id]} #{@info[:vm_id]}_s1 -s 0")
#        @info[:vm].state?("RUNNING")
#
#        wait_loop(:success => "READY", :break => "ERROR") {
...
#        @info[:img_id_pers_save_s1] = cli_create("onevm disk-saveas #{@info[:vm_id]} #{@info[:disk_id_pers]} #{@info[:vm_id]}_pers_s1 -s 0")
#        @info[:vm].state?("RUNNING")
#
#        wait_loop(:success => "READY", :break => "ERROR") {
...
#        @info[:img_id_save_current] = cli_create("onevm disk-saveas #{@info[:vm_id]} #{@info[:disk_id]} #{@info[:vm_id]}_current")
#        @info[:vm].state?("RUNNING")
#
#        wait_loop(:success => "READY", :break => "ERROR") {
...
#        @info[:img_id_save_current_pers] = cli_create("onevm disk-saveas #{@info[:vm_id]} #{@info[:disk_id_pers]} #{@info[:vm_id]}_current_pers")
#        @info[:vm].state?("RUNNING")
#
#        wait_loop(:success => "READY", :break => "ERROR") {
...
######img_id_save_s1 is not created. No support for live snapshots.
