...
describe "VirtualMachine suspend and resume operations test" do
    #---------------------------------------------------------------------------
    ...
    it "should suspend a running VirtualMachine and then, check its state" <<
        " and history" do
        ...
        expect(history(vmxml, 'ACTION').to_i).to eql(10) # Stop/Resume
        ...
        expect(history(vmxml, 'RETIME').to_i).not_to eql(0)
        expect(history(vmxml, 'ESTIME').to_i).to eql(0)
        expect(history(vmxml, 'EETIME').to_i).to eql(0)
    end

    it "should check the Host capacity with a suspended VirtualMachine" do
        vmxml = cli_action_xml("onevm show -x testvm")
    ...
    it "should resume a suspended VirtualMachine and then, check its state" <<
        " and history" do
        ...
        expect(history(vmxml, 'STIME').to_i).to be > 0
        expect(history(vmxml, 'ETIME').to_i).to eql(0)
        expect(history(vmxml, 'PSTIME').to_i).to eql(0)
        expect(history(vmxml, 'PETIME').to_i).to eql(0)
    ...
    it "should delete a suspended VirtualMachine and check the Host capacity" do
        cli_action("onevm suspend testvm")
        @vm.state?("SUSPENDED")

        cli_action("onevm terminate testvm")
