...
mkdir -p samples/vntemplate samples/vntemplate_pool
mkdir -p samples/hook samples/hook_pool
mkdir -p samples/acl samples/acl_pool
mkdir -p samples/security_group samples/security_group_pool
mkdir -p samples/showback
mkdir -p samples/backupjob samples/backupjob_pool
mkdir -p samples/opennebula_configuration
touch output.log

if [ -f ~/xsd.tar.gz ]; then
    cp ~/xsd.tar.gz .
    tar xzf xsd.tar.gz
fi
...
onedatastore create test/datastore.2
...
onevmgroup create test/vm_group.0 --user newuser --password abc
...
onevm backup 0 --datastore dummy_backups
...
# VM samples are saved at the end of the test
...
onezone list -x > samples/zone_pool/2.xml

onezone show 0 -x > samples/zone/1.xml

#VNTemplates
echo "Creating new vntemplate..." >> output.log
onevntemplate list -x > samples/vntemplate_pool/1.xml

onevntemplate create test/vntemplate.0

for i in `onevntemplate list | tail -n +2 | tr -s ' ' | cut -f2 -d ' '`; do
    onevntemplate show $i -x > samples/vntemplate/$i.xml
done

onevntemplate list -x > samples/vntemplate_pool/2.xml

#Hooks
echo "Creating new hooks..." >> output.log
onehook list -x > samples/hook_pool/1.xml

onehook create test/hook.0
onehook create test/hook.1
onehook create test/hook.2

for i in `onehook list | tail -n +2 | tr -s ' ' | cut -f2 -d ' '`; do
    onehook show $i -x > samples/hook/$i.xml
done

onehook list -x > samples/hook_pool/2.xml

#ACL
oneacl list -x > samples/acl_pool/1.xml

echo "<ACL></ACL>" > samples/acl/1.xml

#Security Groups
onesecgroup show -x 0 > samples/security_group/1.xml

onesecgroup list -x > samples/security_group_pool/1.xml

#Backup Jobs
onebackupjob create test/backupjob.0

onebackupjob create test/backupjob.1

onebackupjob lock 0

onebackupjob list -x > samples/backupjob_pool/0.xml

for i in `onebackupjob list | tail -n +2 | tr -s ' ' | cut -f2 -d ' '`; do
    onebackupjob show $i -x > samples/backupjob/$i.xml
done

# VMGroups save samples
...
for i in  cluster datastore group vdc host image vmtemplate user vm vnet vrouter marketplace marketplaceapp vm_group vntemplate hook acl security_group backupjob
do
    POOL_NAME="$i""_pool"
...
oneshowback calculate
oneshowback list -x > samples/showback/1.xml
...
sed -i "s%<SHOWBACK_RECORDS>%<SHOWBACK_RECORDS xmlns='http://opennebula.org/XMLSchema' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xsi:schemaLocation='http://opennebula.org/XMLSchema ../../showback.xsd'>%" samples/showback/*.xml

xmllint --noout --schema acct.xsd samples/acct/* >> output.log 2>&1
xmllint --noout --schema showback.xsd samples/showback/* >> output.log 2>&1

# Config
echo "Dumping configuration" >> output.log
../dump_oned_conf > samples/opennebula_configuration/0.xml

sed -i "s%<OPENNEBULA_CONFIGURATION>%<OPENNEBULA_CONFIGURATION xmlns='http://opennebula.org/XMLSchema' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xsi:schemaLocation='http://opennebula.org/XMLSchema ../../opennnebula_configuraiton.xsd'>%" samples/opennebula_configuration/*.xml

xmllint --noout --schema opennebula_configuration.xsd samples/opennebula_configuration/* >> output.log 2>&1
