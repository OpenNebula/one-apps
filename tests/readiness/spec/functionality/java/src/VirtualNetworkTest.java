...
public class VirtualNetworkTest
{
    ...
    private static String invalid_template =
                        "NAME   = \"Net with invalid AR\"\n" +
                        "BRIDGE = br1\n" +
                        "VN_MAD = dummy\n" +
                        "AR = [ TYPE = IP4, IP=130.10.0.1, SIZE = -1]";

    /**
     *  Wait until the Virtual Network changes to the specified state.
     *  There is a time-out of 5 seconds.
     */
    void waitState(VirtualNetwork vn, String state)
    {
        int n_steps     = 10;
        int step        = 500;
        ...
        while( !( (vn.stateString().equals(state) ) || i > n_steps ))
        {
        ...
        assertTrue( vn.stateString().equals(state) );
    }
     ...
     *  There is a time-out of 5 seconds.
     */
    static void waitDeleted(VirtualNetwork vn)
    {
        int n_steps     = 10;
        int step        = 500;
        ...
        assertTrue("Virtual Network delete timeout, state: " + vn.stateString(),
            res.isError() );
    }
    ...
    public void update()
    {
        ...
        res = vnet.update(new_template, true);
        assertTrue( res.getErrorMessage(), !res.isError() );
    ...
    public void recover()
    {
        res = VirtualNetwork.allocate(client, invalid_template);
        assertTrue( res.getErrorMessage(), !res.isError() );

        VirtualNetwork invalid_vnet =
            new VirtualNetwork(Integer.parseInt(res.getMessage()), client);

        waitState(invalid_vnet, "ERROR");

        res = invalid_vnet.recover(1);
        assertTrue( res.getErrorMessage(), !res.isError() );

        invalid_vnet.info();
        assertTrue( invalid_vnet.stateString().equals( "READY" ) );
