...
RSpec.describe "Scheduling requirements tests" do
    ...
    # TESTS
    #---------------------------------------------------------------------------
    it "should allocate VMs considering HOST-VM affinity rules" do
        template = <<-EOF
            NAME = host-vm
        ...
        5.times { |i|
            vmid, vm = vm_with_group("0.1", "host-vm", "aff")
        ...
        vms.each { |v|
            ...
            expect(v.hostname).to satisfy { |j| j == "host0" || j == "host4" }
        ...
        5.times { |i|
            vmid, vm = vm_with_group("0.1", "host-vm", "anti")
        ...
        vms.each { |v|
            ...
            expect(v.hostname).to satisfy { |j| j != "host2" && j != "host3" }
    ...
    it "should allocate VMs considering VM-VM intra-role affinity rules" do
        ...
        5.times { |i|
            vmid, vm = vm_with_group("0.1", "vm-vm", "aff")
        ...
        5.times { |i|
            vmid, vm = vm_with_group("0.1", "vm-vm", "anti")
