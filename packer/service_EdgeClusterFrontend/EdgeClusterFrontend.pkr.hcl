...
# Build VM image
source "qemu" "EdgeClusterFrontend" {
  cpus        = 2
  memory      = 2048
  accelerator = "kvm"
...
build {
  sources = ["source.qemu.EdgeClusterFrontend"]
  ...
  provisioner "file" {
    sources     = ["appliances/EdgeClusterFrontend/appliance.sh"]
    destination = "/etc/one-appliance/service.d/"
  }

  # Copy Edge Cluster Frontend source code into the image
  provisioner "file" {
    source      = "/root/cognit-appliance-building/edgecluster-frontend"
    destination = "/root/"
  }

  # Copy Prometheus RabbitMQ Exporter source code into the image
  provisioner "file" {
    source      = "/root/cognit-appliance-building/prometheus-rabbitmq-exporter"
    destination = "/root/"
  }
