...
source "qemu" "sles" {
  ...
  iso_url      = lookup(lookup(var.sles, "${var.version}.${var.arch}", {}), "iso_url", "")
  iso_checksum = lookup(lookup(var.sles, "${var.version}.${var.arch}", {}), "iso_checksum", "")
...
build {
  ...
  provisioner "shell" {
    ...
    environment_vars = [
      ...
      "DIST_ARCH=${var.arch}",
      "SLE_REGCODE=${var.sles_regcode}",
      "SLE_EMAIL=${var.sles_email}"
    ]
