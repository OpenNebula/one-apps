...
BASE_PATH     = '/etc/one-appliance/service.d/vllm-engine'
VLLM_LOG_FILE = '/var/log/one-appliance/vllm.log'
WEB_PATH      = '/etc/one-appliance/service.d/vllm-engine/client'
...
#  Defaults to "0.9".
#
# Note: vllm-engine REQUIRES a model disk to be attached. The appliance will:
#   - Auto-detect the first non-OS disk (block device or virtiofs)
#   - Determine the disk format (qcow2/block vs dir/virtiofs)
#   - Mount to /mnt/{disk_device} or /mnt/{virtiofs_tag}
#   - Use the mounted path as the model source
#   - If no model disk is found, the appliance will fail to start
# ------------------------------------------------------------------------------
...
def gen_web_config(model_path)
    # model_path is mandatory for vllm-engine (no HuggingFace fallback)
    unless model_path && File.directory?(model_path) && !Dir.empty?(model_path)
        raise "Model path is required but not provided or invalid: #{model_path}"
    end
