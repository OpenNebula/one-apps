...
ONE_SERVICE_PARAMS=(
    'ONEAPP_MINIO_ROOT_USER'           'configure'  'MinIO root user for MinIO server'                              ''
    'ONEAPP_MINIO_ROOT_PASSWORD'       'configure'  'MinIO root user password for MinIO server'                     ''
    'ONEAPP_MINIO_OPTS'                'configure'  'Additional commandline options for MinIO server'               ''
)
...
service_configure()
{
    ...
    if ! mount -a -t xfs; then
        g error "Error mounting MinIO drives"
    fi
...
install_minio_server()
{
    ...
    if ! dpkg -i minio.deb ; then
        msg error "Error installing MinIO"
        exit 1
    fi
