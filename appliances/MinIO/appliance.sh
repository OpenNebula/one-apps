...
MINIO_HOSTNAME="${ONEAPP_MINIO_HOSTNAME:-localhost,minio-*.example.net}"
MINIO_TLS_CERT="${ONEAPP_MINIO_TLS_CERT}"
MINIO_TLS_KEY="${ONEAPP_MINIO_TLS_KEY}"
...
service_configure()
{
    ...
    msg info "Value for TLS certificate: ${MINIO_TLS_CERT}"
    msg info "Value for TLS key: ${MINIO_TLS_KEY}"
    if [ -z "${MINIO_TLS_CERT}" ] || [ -z "${MINIO_TLS_KEY}" ]; then
        msg info "Autogenerating TLS certificates..."
        generate_tls_certs
    else
        msg info "Configuring provided TLS certificates..."
        echo ${MINIO_TLS_CERT} | base64 --decode >> /opt/minio/certs/public.crt
        echo ${MINIO_TLS_KEY} | base64 --decode >> /opt/minio/certs/private.key
    fi
...
generate_tls_certs()
{
    ...
    msg info "Give ownership or certificates to minio-user"
